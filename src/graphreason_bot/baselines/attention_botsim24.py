"""BotSim-24 Attention prompt baseline using the main experiment's LLM client.

This script adapts the official-style Attention structure prompt to the
BotSim-24 test file format. It runs all three LLMs by default and stores
outputs under the repository's ignored ``outputs/baselines/attention`` folder.
"""
...
def active_years(created_at: str, reference_year: int = 2023) -> str:
    # Match the official Random/Attention prompting code's 2023 reference year.
...
def checkpoint_path(
    ...
    return (
        RESULT_ROOT
        / "checkpoints"
        / f"botsim-24_{model}_attention_{run_tag(source, sample_seed, sample_size, balanced_sample)}_{scope}.jsonl"
    )
...
def result_path(
    ...
    return (
        RESULT_ROOT
        / f"botsim-24_{model}_attention_{run_tag(source, sample_seed, sample_size, balanced_sample)}_{suffix}.json"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Attention prompting on BotSim-24 with three LLMs."
    )
    parser.add_argument(
        "--model", default="all",
    ...
    summary_path = RESULT_ROOT / f"botsim-24_all_models_attention_{tag}_{suffix}.json"
    write_json(
        summary_path,
        {
            "method": "Attention",
