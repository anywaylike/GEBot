"""TwiBot-22 Random prompt baseline using the main experiment's LLM client.

This module uses the same LLM client and metric implementation as the main
experiment while remaining independent from the current working directory.
"""
...
def checkpoint_path(
    ...
    return (
        RESULT_ROOT
        / "checkpoints"
        / f"twibot-22_{model}_random_{run_tag(source, sample_seed, sample_size, balanced_sample)}_{scope}.jsonl"
    )
...
def run_model(
    ...
    for position, item in enumerate(selected_items, start=1):
        ...
        if target_id in completed:
            ...
            print(
                f"[Random] {model} {position}/{len(selected_items)} "
                f"target_id={target_id}: true={resumed_record['gold']}, "
                f"pred={resumed_record['prediction']} [resumed]"
            )
            continue
        ...
        if dry_run:
            ...
            # This is the same OpenAI-compatible interface used by the main
            # experiment, including its model IDs, temperature and retries.
        ...
        print(
            f"[Random] {model} {position}/{len(selected_items)} "
            f"target_id={target_id}: true={record['gold']}, "
...
def result_path(
    ...
    return (
        RESULT_ROOT
        / f"twibot-22_{model}_random_{run_tag(source, sample_seed, sample_size, balanced_sample)}_{suffix}.json"
    )


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Random prompting on TwiBot-22 with three LLMs."
    )
    parser.add_argument(
        "--model", default="all",
        help=(
    ...
    summary_path = RESULT_ROOT / f"twibot-22_all_models_random_{tag}_{suffix}.json"
    write_json(
        summary_path,
        {
            "method": "Random",
